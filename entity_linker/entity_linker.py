...
from util import normalize_entity_name, remove_prefixes_from_name, remove_suffixes_from_name
...
class KBEntity(Entity):
    ...
    def __repr__(self):
        ...
        return '(%s score=%f %s)' % (self.id, self.score, aliases_str)
        # return '(' + str(self.id) + ', ' + self.score + ',' + aliases_str + ')'
...
class IdentifiedEntity():
    ...
    def is_entity(self):
        return self.get_mid().startswith('m.')
    ...
    def get_mid(self):
        if isinstance(self.entity, KBEntity):
    ...
    def overlap_with(self, other):
        return len(set(self.tokens) & set(other.tokens)) > 0

    def __repr__(self):
        tokens_str = ', '.join(map(lambda x: x.__repr__(), self.tokens))
        return ('%s, %s, %s, surface score=%f, popularity score=%f, perfect_match=%s' %
                (self.entity.__repr__(), self.name, tokens_str, self.surface_score, self.score, self.perfect_match)).encode('utf8')
        # return self.entity.__repr__() + ', ' + self.name + '\n ' + tokens_str + ', surface score' \
        #        + self.surface_score + ', popularity score:' + self.score + ', perfect match:' + self.perfect_match
...
class EntityLinker:
    ...
    def is_entity_occurrence(self, tokens, start, end):
        ...
        elif len(pos_list) == 1 and (pos_list[0].startswith('N') or pos_list[0].startswith('JJ')) \
    ...
    def is_entity_occureence_fallback(self, tokens, start, end):
        # Concatenate POS-tags
        token_list = tokens[start:end]
        pos_list = [t.pos for t in token_list]
        pos_str = ''.join(pos_list)
