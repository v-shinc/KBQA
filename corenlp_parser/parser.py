...
class CoreNLPParser(object):
    ...
    def parse(self, query, include_json=False,
        ...
        response = self.server.parse(query)
...
def extract_tokens_from_json(result):
    ...
    for i, (w, info) in enumerate(words):
        t = Token(w)
        t.index = i
        t.pos = info['PartOfSpeech']
        t.lemma = info['Lemma']
        if 'norm-ner' in info:
            t.nne_tag = info['norm-ner']
        if 'ner' in info:
            t.ne_tag = info['ner']
        tokens.append(t)
    return tokens
...
class Token(object):
    ...
    def __repr__(self):
        return '(%s: token=%s, pos=%s, lemma=%s' \
               % (self.index, self.token, self.pos, self.lemma)
    # return 'token=%s, pos=%s, lemma=%s, index=%s, ne_tag=%s, nne_tag=%s, dep_node=%s' \
    #            % (self.token, self.pos, self.lemma, self.index, self.ne_tag, self.nne_tag, self.dep_node)
