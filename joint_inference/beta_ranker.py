...
class BetaRanker:
    ...
    def __init__(self, params):
        ...
        self.pattern_word_ids = [
            tf.placeholder(tf.int32, [None, max_sentence_len], name=('pattern_word_ids_%s' % i))
            for i in range(2)]
        self.sentence_lengths = [tf.placeholder(tf.int32, [None], name=('sentence_lengths_%s' % i)) for i in range(2)]
        ...
        self.relation_ids = [tf.placeholder(tf.int32, [None, 3], name=('relation_ids_%s' % i)) for i in range(2)]
        self.mention_char_ids = [tf.placeholder(tf.int32, [None, params['max_name_len']], name=('mention_char_ids_%s' % i))
                                 for i in range(2)]
        self.topic_char_ids = [tf.placeholder(tf.int32, [None, params['max_name_len']], name=('topic_char_ids_%s' % i))
                               for i in range(2)]
        self.extras = [tf.placeholder(tf.float32, [None, len(params['extra_keys'])], name=('extra_%s' % i))
                       for i in range(2)]
        self.dropout_keep_prob = tf.placeholder(tf.float32, name='dropout_keep_prob')

        with tf.device('/gpu:%s' % params.get('gpu', 1)):
            if params['relation_encoder'] == 'CNN':
                pattern_encoder = encoder.CNNEncoder(params['question_config'], 'question_cnn')
                ...
                pattern_encoder = encoder.RNNEncoder(params['question_config'], 'question_rnn')
            ...
            topic_mention_scores = [self.cosine_sim(topics[i], mentions[i]) for i in range(2)]
            ...
            # Concat features
            features = [tf.concat(1, [pat_drops[i], rel_drops[i], bi_sims[i], topics[i], mentions[i], topic_mention_scores[i], self.extras[i]]) for i in range(2)]
            ...
            self.train_op = optimizer_map['optimizer'](params['lr']).minimize(self.loss)
    ...
    def fit(self,
        ...
        if 'word_dim' in self.params['question_config']:
            feed_dict[self.pattern_word_ids] = pattern_word_ids
            feed_dict[self.sentence_lengths] = sentence_lengths

        if 'char_dim' in self.params['question_config']:
            feed_dict[self.pattern_char_ids] = pattern_char_ids
            feed_dict[self.word_lengths] = word_lengths

        feed_dict[self.dropout_keep_prob] = dropout_keep_prob
        feed_dict[self.relation_ids] = relation_ids
        feed_dict[self.mention_char_ids] = mention_char_ids
        feed_dict[self.topic_char_ids] = topic_char_ids
        feed_dict[self.extras] = extras
